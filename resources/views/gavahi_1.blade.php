...
<body> {{--{{ var_dump($data) }}--}}@foreach($data as $key=>$item)
    <table>
        <tr>
            ...
            <td style="width:250px;text-align: center">
                <div class="barcodecell">
                    <barcode code={{$item["barcode"]}} type="C128C" class="barcode" size="1" height="1"/>
                </div>
                <div><span
                        style="margin-right:10em;font-size: 18px"> (اصالت گواهی با استعلام این بارکد تعیین می شود) </span>
                </div>
            </td>
        </tr>
    </table>
    <table class="table2" style="border-collapse: collapse">
        ...
        <tr>
            <td style="width:100px;text-align: right;margin-left:0"><span
                    style="width:100px;text-align: right;margin-left:0">کد پستی ده رقمی:</span>
            </td> @for($i=count($item["postalcode"]) - 1;$i>=0;$i--)
                <td class="border">{{$item["postalcode"][$i]}}</td> @endfor
            <td></td>
    ...
    <table>
        ...
        <tr>
            <td>پلاک:</td>
            <td style="width:230px">{{$item["plate_no"]}}</td>
    ...
    <div>
        <table>
            <tr>
                <td style="width:70%;border-top:4px solid black"> قابل توجه درخواست کننده گرامی:<br>۱.کدپستی مندرج در
                    گواهی منحصرا متعلق به نشانی ذکر شده بوده و استفاده از این کد پستی برای سایر مکانها موجب اشتباه در
                    پردازش اطلاعات و عدم خدمت رسانی صحیح به شما خواهد بود<br>۲.مدت اعتبار این گواهی از تاریخ
                    صدور {{$ttl}} روز می باشد.<br>۳.با مراجعه به سامانه GNAF.POST.IR می توانید کد پستی خود را دریافت
                    کنید.<br>۴.هزینه صدور گواهی فوق {{$price}} ریال می باشد.<br>۵.اصالت این گواهی از طریق استعلام بر خط
                    بارکد تعیین می گردد.<br>۶.در صورت مشاهده هر گونه مغایرت در اطلاعات این گواهی با نشانی خود, به
                    نزدیکترین دفتر پستی مراجعه نمایید.
                </td>
                <td style="width:30%;border-top:4px solid black;border-right:4px solid black;text-align: center">
                    <barcode code={{$QRCode}} type="QR" class="barcode" size="1.5" error="M" height="2"
                             disableborder="1"/>
                </td>
            </tr>
        </table>
